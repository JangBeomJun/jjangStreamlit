...
def button_callback():
    if radio_selected_lang== "한국어":
        translated_text = image_gen.translate_text_for_image(input_text)
    elif radio_selected_lang == "영어":
        translated_text = input_text
        
    if detail_description == 'Yes':
        resp = image_gen.generate_text_for_image(translated_text)
        text_for_image = resp
    ...
    image_urls = image_gen.generate_image_from_text(text_for_image, image_num, image_size)
    images = []
    download_file_names = []
    for k,image_url in enumerate(image_urls):
        ...
        download_file_name = f"gen _image_{k}_{now_datetime}.png"
...
image_num = st.sidebar.radio('생성할 이미지의 개수 지정',image_num_options,index=0, horizontal=True)
